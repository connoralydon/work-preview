...
INSERT INTO previews (id, prefix, port, public, status, created_at, last_access_at, expires_at)
VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, p.ID, p.Prefix, p.Port, p.Public, p.Status, p.CreatedAt, p.LastAccessAt, p.ExpiresAt)
...
SELECT id, prefix, port, repository, branch, commit_hash, public, status, created_at, last_access_at, expires_at
FROM previews WHERE status = 'active' ORDER BY created_at`)
	...
	for rows.Next() {
		var p Preview
		if err := rows.Scan(&p.ID, &p.Prefix, &p.Port, &p.Repository, &p.Branch, &p.Commit, &p.Public, &p.Status, &p.CreatedAt, &p.LastAccessAt, &p.ExpiresAt); err != nil {
			return nil, err
		}
...
FROM previews WHERE id = ? AND status = 'active'`, id).Scan(
		&p.ID, &p.Prefix, &p.Port, &p.Repository, &p.Branch, &p.Commit, &p.Public, &p.Status, &p.CreatedAt, &p.LastAccessAt, &p.ExpiresAt,
	)
